...
class EnableAll extends AbstractModuleManagerTests
{
    ...
    protected function checkMonologRecords(): bool
    {
        ...
        if (count($this->logRecords) !== 6) {
            BasicMsg::displayMsgNL('Fail : Number of line not equal to 6', 'red', 'bold');
            return false;
        }
        ...
        try {
            ...
            //Line 2 [2019-05-22 22:33:19] bfw.DEBUG: FileManager - Create symlink - Use relative path {"target":"../available/bfw-hello-world/src/"} []
            $lineNb++;
            $this->checkLogLineMsg($lineNb, 'FileManager - Create symlink - Use relative path');
            $this->checkLogLineContextKeys($lineNb, ['target']);
            $this->checkLogLineContextKeyContain($lineNb, 'target', '../available/bfw-hello-world/src/');

            //Line 3 [2019-05-17 10:03:03] bfw.DEBUG: Module - Read module info {"name":"bfw-test-install","path":"/opt/Projects/bfw/bfw/test/install/app/modules/available/bfw-test-install"} []
            $lineNb++;
            $this->checkLogLineMsg($lineNb, 'Module - Read module info');
            $this->checkLogLineContextKeys($lineNb, ['name', 'path']);
            ...
            //Line 4 [2019-05-17 10:03:03] bfw.DEBUG: FileManager - Create symlink {"linkTarget":"/opt/Projects/bfw/bfw/test/install/app/modules/available/bfw-test-install/src/","linkFile":"/opt/Projects/bfw/bfw/test/install/app/modules/enabled/bfw-test-install"} []
            $lineNb++;
            $this->checkLogLineMsg($lineNb, 'FileManager - Create symlink');
            $this->checkLogLineContextKeys($lineNb, ['linkTarget', 'linkFile']);
            ...
            //Line 5 [2019-05-22 22:33:19] bfw.DEBUG: FileManager - Create symlink - Use relative path {"target":"../available/bfw-test-install/src/"} []
            $lineNb++;
            $this->checkLogLineMsg($lineNb, 'FileManager - Create symlink - Use relative path');
            $this->checkLogLineContextKeys($lineNb, ['target']);
